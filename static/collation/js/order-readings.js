...
var OR = (function () {
	...
	return {
		...
		show_reorder_readings: function (options) {
			...
			SimpleContextMenu.attach('deletable', function () {return OR.make_menu('deletable_unit')});
		...
		approve_verse: function () {
			...
			if (!standoff_problems[0]) {
				...
				if (extra_results[0] === true) {
					...
					delete CL._data.event_list;
					OR.uniqueify_ids();
					OR.order_witnesses_for_output();
					SR._lose_subreadings(); //always lose before finding
		...
		order_witnesses_for_output: function () {
						...
						for (i = 0; i < CL._data[key].length; i += 1) {
							for (j = 0; j < CL._data[key][i].readings.length; j += 1) {
								...
								if (CL._data[key][i].readings[j].hasOwnProperty('subreadings')) {
									for (type in CL._data[key][i].readings[j].subreadings) {
										for (k = 0; k < CL._data[key][i].readings[j].subreadings[type].length; k += 1) {
											CL._data[key][i].readings[j].subreadings[type][k].witnesses = CL.sort_witnesses(CL._data[key][i].readings[j].subreadings[type][k].witnesses);
										}
									}
								}
							}
			...
			CL._data['lac_readings'] = CL.sort_witnesses(CL._data['lac_readings']);
			CL._data['om_readings'] = CL.sort_witnesses(CL._data['om_readings']);
		},

		are_all_empty_readings: function (readings) {
			var i;
		...
		get_unit_data: function (data, id, format, start, end, options) {
			...
			if (OR.are_all_empty_readings(data) && !options.hasOwnProperty('created')) {
		...
		make_main_reading: function (id_string) {
			...
			SV.prepare_for_operation({'app_id': app_id, 'unit_id': unit._id});
		...
		_add_CM_Handlers: function () {
			...
			if (document.getElementById('delete_unit')) {
				...
				$('#delete_unit').on('click.du_c', function (event) {
					...
					element = SimpleContextMenu._target_element;
					div = CL._get_specified_ancestor(element, 'DIV', function (e) { if ($(e).hasClass('spanlike')) {return false;} return true;});
					OR.get_delete_unit(div)
				});
				$('#delete_unit').on('mouseover.du_mo', function(event) {CL.hide_tooltip();});
			}
		...
		//we added the correct start an end values in OR.horizontal_combine_overlaps

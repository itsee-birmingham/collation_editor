...
var local_services = {
	...
	get_verse_data : function (verse, witness_list, private_witnesses, success_callback) {
		...
		if (private_witnesses) return success_callback([], RG.calculate_lac_wits);
	...
	//check this works - it doesn't seem to get global rules - should it?
	...
	get_rules_by_ids : function(ids, result_callback, rules, i) {
	    ...
	    var rule_type = ids[i].split('_')[0];
	    var resource_type = 'project/'+CL._project._id
	    + '/rule'
	...
	update_ruleset : function (for_deletion, for_addition, verse, success_callback, i, j) {
		...
		if (i < for_deletion.length) {
			local_services._delete_resource(local_services._get_rule_type(for_deletion[i], verse), function () {
				return local_services.update_ruleset(for_deletion, for_addition, verse, success_callback, ++i, j);
			});
		}
		else if (j < for_addition.length) {
			if (typeof for_addition[j]._id === 'undefined') for_addition[j]._id = (for_addition[j].scope === 'always' ? 'global_' : ('verse_'+verse+'_')) + U.generate_uuid();
		...
		else if (success_callback) success_callback();
	},

...
//	_display_settings: {'view_supplied': true, 'view_unclear': true, 'view_punctuation': false, 'view_capitalisation': false, 'view_diaeresis': false, 'view_apostrophes': false, 'expand_abbreviations': true, 'view_original_forms': true},
	...
	get_overlap_layout: function (apparatus, app, format, overtext_length, options) {
		...
		if (unit_index === null || i < unit_index) { //we don't have a variant for this word
			...
			} else if (format === 'version' || format === 'version_additions') {
	...
	add_subreading_events: function (stage, base_subreading_rules) {
		...
		if (document.getElementById('show_hide_subreadings_button')) {
			...
			if (typeof base_subreading_rules !== 'undefined') {
			    SV._find_subreadings({'rule_classes': base_subreading_rules});
			}
			if (stage === 'reorder') {			    
		...
		if (document.getElementById('show_hide_subreadings_button')) {
		    document.getElementById('show_hide_subreadings_button').value = document.getElementById('show_hide_subreadings_button').value.replace('hide', 'show');
    			...
    			SV._lose_subreadings();//this is needed because if we have any showing (for example in order readings) and we try to fun find_subreadings we will lose the ones already showing
...
//    			}
	...
	/** 
	 * Removes and units which only contain om/lac readings from the data structure
	 * 
	...
	add_extra_gap_readings: function (adjacent_unit, all_witnesses, new_unit, check_overlaps, inclusive_overlaps) {
	    ...
	    if (typeof check_overlaps 
			&& adjacent_unit.hasOwnProperty('overlap_units') 
		...
		for (key in adjacent_unit.overlap_units) {
			...
			for (j = 1; j < ol_unit.readings.length; j += 1) {
			    new_rdg = CL.get_new_gap_rdg_details(adjacent_unit, ol_unit.readings[j]);
			    new_rdg.text = [];
	...
	create_extra_gaps: function () {
		...
		if (i === 0) { //then we are in the very first unit and if we have gaps before then we must add a unit
			...
			dict = {'start': 1, 
				...
				'readings': []};
			dict = CL.add_extra_gap_readings(apparatus[i], all_witnesses, dict, true);
		...
		if (CL.extra_gap_required_after(apparatus, i)) {
			...
			dict = CL.add_extra_gap_readings(apparatus[i], all_witnesses, dict, true, inclusive_overlaps);
			extras['' + (i+1)] = dict;
		    } else {
			//otherwise we don't need to worry about overlaps
			temp = CL.get_extra_gap_location(apparatus[i].end);
			dict = {'start': temp['start'], 
				'end': temp['end'], 
				'created': true,
				'first_word_index': temp['first_word_index'],
				'readings': []};
			dict = CL.add_extra_gap_readings(apparatus[i], all_witnesses, dict, false);
			extras['' + (i+1)] = dict;
		    }
		}
	    }
	...
	extract_witness_text: function (reading, options, test) {
	    ...
	    if (typeof witness === 'undefined' && reading.witnesses.length === 1  && !reading.hasOwnProperty('created')) {
		...
		if (reading.text[0].hasOwnProperty(witness) && reading.text[0][witness].hasOwnProperty('gap_before')) { //first unit in verse this might actually work like the others now
        	...
        	} else if (reading.text[0].hasOwnProperty('combined_gap_before_details')) { //subsequent units in verse
...
//	    word = '%s%s' % (word, token['pc_after'])
	...
	get_reading_witnesses: function (data, app_id, start, end, with_suffixes) {
		...
		witnesses.push.apply(witnesses, VER.get_reading_witnesses(unit_id, data._id))
	    }
	    return witnesses;
	},
	...
	add_triangle_functions: function (format) {
        ...
        show_split_witness_menu: function (reading, menu_pos, details) {
            ...
            left = menu_pos.left;
            top = menu_pos.top;
            //if there is already an old meny hanging around remove it
        ...
        mark_standoff_reading: function (type, name, reading_details, format, menu_pos) {
            var i, reading, parents, unit, new_reading_id, subreading_classes;
        	...
        	var extra_details, data, witness_list, key;
...
//            console.log(JSON.parse(JSON.stringify(CL._data)))
        ...
        do_make_standoff_reading: function (type, apparatus, unit, reading, parent) {
            var i, rule_details, reading_text, details, existing_standoff, position, total, temp, type_string;
            SV._lose_subreadings(); //needs to be done so the main reading we are dealing with contains *all* witnesses including those of subreadings
...
/* Menu Loading */
        ...
        set_display_settings: function (project) {
        	...
        	for (i = 0; i < project.display_settings.configs.length; i += 1) {
        	    ...
        	    CL._display_settings[project.display_settings.configs[i].id] = project.display_settings.configs[i].check_by_default;
                }
